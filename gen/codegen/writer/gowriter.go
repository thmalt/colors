...
func (w *GoWriter) SetBuildTags(tags ...string) {
	w.buildTags = append(w.buildTags[:0], tags...)
}

func (w *GoWriter) AddBuildTags(tags ...string) {
	w.buildTags = append(w.buildTags, tags...)
}

func (w *GoWriter) BuildTags() string {
	var s strings.Builder

	for _, tag := range w.buildTags {
		...
		if strings.ContainsAny(tag, "&|,") {
			s.WriteByte('(')
			s.WriteString(strings.ReplaceAll(tag, ",", " && "))
			s.WriteByte(')')
		} else {
			s.WriteString(tag)
		}
...
func (w *GoWriter) SaveGoFile(filename, pkg string, forceWrite bool) bool {
	data, err := w.buildGo(pkg)
	if err != nil {
		log.Printf("Error buildGo: %v", err)
	}
	...
	if err := os.WriteFile(filename, data, 0o644); err != nil {
		log.Printf("Could not write file %s: %v", filename, err)
		return false
	}
...
func (w *GoWriter) WriteGo(out io.Writer, pkg string) (int, error) {
	data, err := w.buildGo(pkg)
	if err != nil {
		log.Printf("Error buildGo: %v", err)
	}
...
func (w *GoWriter) buildGo(pkg string) ([]byte, error) {
	...
	header := fmt.Sprintf("// Code generated by \"go generate %s\" in %s. DO NOT EDIT.", w.generatorPath, w.module)

	return formatCode(pkg, w.BuildTags(), header, buf.Bytes(), w.formatSource)
}
